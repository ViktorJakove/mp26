...
export function createBankManager(app, getMoney, addMoney, subMoney) {
    ...
    function showLoanUI(panel, desc, instr, sprite, onComplete, options) {
        ...
        const statusText = new PIXI.Text(
            loanActive ? "ZATIM XYXYXY" : "Nemáš žádný dluh",
            {
                fontFamily: "Arial",
                fontSize: 18,
                fill: 0xecf0f1,
        ...
        if (!loanActive) {
            ...
            const confirmBtn = createButton("Půjčit", 0x27ae60, 0x2ecc71, panelW/2 - 75, 260, 150, 45, () => {
                if (htmlInput) {
                    ...
                    if (amount && amount > 0 && amount <= bankOptions.maxLoan) {
                        ...
                        desc.text = `Půjčeno $${amount}. Nezapomeň vrátit $${amount * bankOptions.interestRate}! Čas běží!`;
                ...
                if (panel && uiContainer) {
                    createHTMLInput(panel, inputX, inputY, inputWidth, (value) => {
                        if (value && value > 0 && value <= bankOptions.maxLoan) {
                            ...
                            desc.text = `Půjčeno $${value}. Nezapomeň vrátit ${bankOptions.interestRate}× ($${value * bankOptions.interestRate})!`;
            ...
            const repaymentText = new PIXI.Text(
                `Pro splacení dluhu potřebuješ $${repaymentAmount}`,
                {
                    fontFamily: "Arial",
                    fontSize: 18,
                    fill: hasEnoughMoney ? 0x2ecc71 : 0xe74c3c, //zelena : cervena
                    align: "center",
                    wordWrap: true,
                    wordWrapWidth: panelW - 60
                }
            ...
            if (hasEnoughMoney) {
                const repayBtn = createButton("Splatit dluh", 0x27ae60, 0x2ecc71, panelW/2 - 100, 210, 200, 50, () => {
                    ...
                    desc.text = "Dluh úspěšně splacen! Děkujeme.";
